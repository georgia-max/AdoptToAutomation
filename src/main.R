...
# the prepare_data.R to preprocess the data. 
# The preprocess dataset is stored in Data/CRIPTON_60_MINUTES_anonymized_all_prepare.csv
...
# Load utilities
# source("src/Plot.R") #Comment for now. 
source("utils/Functions.R")

# Define variables
# independent_vars_Manual <- c("TotalWL", "Manual_fraction", "PHONE", "MESSAGE_OTHER_count", "Hour")

all_vars <- c("Error_Count", "Error_rate", "TotalWL", "Auto_fraction", "Auto_fraction_sq", 
              "PHONE", "JUSTIF_count", "MESSAGE_OTHER_count",
              "Hour", "DATE_EVENT", "RSE_ID_ANONYM","hours_worked", "TRAF_COMP", "TRAF_DENS_1")

# Data preparation --------------------
regress_df <- df[, all_vars]

# Create summary table
final_stats <- as.data.frame(describe(regress_df))
print(final_stats)
# write.csv(final_stats, file = "./Result/sumstats_091923.csv")

# Unique count of RSE_ID_ANONYM
...
# Define regression formulas (Error Rate Model) ---------------------- 

formula1 <- make_formula("Error_rate", c("TotalWL", "Auto_fraction", "PHONE", "JUSTIF_count", "MESSAGE_OTHER_count", "Hour", "TRAF_COMP"), "DATE_EVENT + RSE_ID_ANONYM")
formula2 <- make_formula("Error_rate", c("TotalWL", "Auto_fraction", "Auto_fraction_sq", "TotalWL_sq", "PHONE", "JUSTIF_count", "MESSAGE_OTHER_count", "Hour", "TRAF_COMP"), "DATE_EVENT + RSE_ID_ANONYM")
formula3 <- make_formula("Error_rate", c("TotalWL", "Auto_fraction", "TotalWL*Auto_fraction", "Auto_fraction_sq", "TotalWL_sq", "Auto_fraction_sq*TotalWL_sq", "PHONE", "JUSTIF_count", "MESSAGE_OTHER_count", "Hour", "TRAF_COMP"), "DATE_EVENT + RSE_ID_ANONYM")

# Run regressions
m1.1 = feols(formula1, vcov = "twoway", data = df)
m1.2 = feols(formula2, vcov = "twoway", data = df)
m1.3 = feols(formula3, vcov = "twoway", data = df)
...
regression_table <- modelsummary(
  models_list,
  stars = TRUE,
  title = "Error Rate OLS Regression Table",
  output = "gt",
