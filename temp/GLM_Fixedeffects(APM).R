...
nb= fenegbin(Error_Count ~ Manual_count + MOVE_AUTO_count 
...
m1= fenegbin(Error_Count ~ Manual_count + MOVE_AUTO_count+ 
               #CHANGE_AUTO_AA_count + CHANGE_AUTO_WW_count+ #Exo
             PHONE + JUSTIF_count + MESSAGE_OTHER_count + Hour #Control Var
...
m2= fenegbin(Error_Count ~ Manual_count + MOVE_AUTO_count 
             ...
             + I(Manual_count^2) + I(MOVE_AUTO_count^2)
                  # + I(CHANGE_AUTO_AA_count^2)+ I(CHANGE_AUTO_WW_count^2)
                   | DATE_EVENT+RAILWAY_DAYS+AGE,vcov = "twoway", data = df
)
...
m2.ins= fenegbin(Error_Count ~ Manual_count + MOVE_AUTO_count 
                 ...
                 + csw(I(Manual_count^2) + I(MOVE_AUTO_count^2))
                       #+ I(CHANGE_AUTO_AA_count^2)+ I(CHANGE_AUTO_WW_count^2))
                 #+MOVE_MAN_resi+MOVE_AUTO_resi+I(MOVE_MAN_resi^2)+I(MOVE_AUTO_resi^2) #Residual -1
                 | DATE_EVENT+RAILWAY_DAYS+AGE,vcov = "twoway", data = df_hat)
...
cap <- 'Model2 Results -NB regreession'
#omit<- 'RMSE|BIC'
estimate <- "{estimate} ({std.error}){stars}"
...
tab<- modelsummary(models, stars = TRUE,  output = "gt", estimate  = estimate,#gof_map =gof_map,
                   #coef_map = cm,
                   statistic = NULL, coef_omit = "theta",
                   title=cap
                  # , gof_omit = omit
...
#library("webshot2")
gt::gtsave(tab,filename = "APM_results/2stage_Model2_APM.docx")
...
# predict.data <- setNames(data.frame(matrix(0, ncol = 8, nrow =1000)), c('Manual_count','MOVE_AUTO_count', 'JUSTIF_count','PHONE' 
...
# predict.data<- predict_X(Manual_count, m2_qp, predict.data )
# dev.off()
# #source("Plot.R", print.eval=TRUE)
# #png(filename="MVE_MAN.png")

# plt<- plotCI(predict.data, m_qp, Manual_count,"Manual Count")
...
#########
library(fixest)
# m2= fenegbin(Error_Count ~ Manual_count + MOVE_AUTO_count #+ CHANGE_AUTO_AA_count + CHANGE_AUTO_WW_count 
#              + PHONE + JUSTIF_count + MESSAGE_OTHER_count + Hour#Control Var
#              + I(MOVE_MAN_count^2) + I(MOVE_AUTO_count^2)#+ I(CHANGE_AUTO_AA_count^2)+ I(CHANGE_AUTO_WW_count^2))-1
#                    | DATE_EVENT+AGE+RAILWAYS_DAYS,vcov = "twoway", data = df
# )
library("MASS")
