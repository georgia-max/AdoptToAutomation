...
#Testing on APM Dataset 


nb= fenegbin(Error_Count ~  + MOVE_AUTO_count 
             + PHONE + JUSTIF_count + MESSAGE_OTHER_count + Hour#Control Var
             + csw(I(^2) + I(MOVE_AUTO_count^2))-1| DATE_EVENT+RAILWAY_DAYS+AGE,vcov = "twoway", data = df
)

po= fepois(Error_Count ~  + MOVE_AUTO_count 
           + PHONE + JUSTIF_count + MESSAGE_OTHER_count + Hour#Control Var
           + csw(I(^2) + I(MOVE_AUTO_count^2))-1| DATE_EVENT+RAILWAY_DAYS+AGE,vcov = "twoway", data = df
)
...
qupoi= feglm(Error_Count ~  + MOVE_AUTO_count 
             + PHONE + JUSTIF_count + MESSAGE_OTHER_count + Hour#Control Var
             + csw(I(^2) + I(MOVE_AUTO_count^2))-1| DATE_EVENT+RAILWAY_DAYS+AGE,family = "quasipoisson", vcov = "twoway", data = df
)
...
#library("webshot2")
gt::gtsave(tab,filename = "APM_results/APM_threemodel.docx")
